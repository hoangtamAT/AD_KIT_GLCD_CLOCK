...
void tempDisplay(unsigned char T_x, unsigned char T_y, unsigned char H_x, unsigned char H_y)
{
  ...
  #asm("sei"); 
  sprintf(lcdBuff,"%2.0f",temp);
  glcd_setfont(font5x7);
  glcd_outtextxy(T_x,T_y+3,lcdBuff);
  glcd_outtextxy(T_x+12,T_y,"o");
  glcd_outtextxy(T_x+18,T_y+3,"C");
  
  sprintf(lcdBuff,"%2.0f",humi);
  glcd_setfont(font5x7);
  glcd_outtextxy(H_x,H_y+3,lcdBuff);
  glcd_outtextxy(H_x+12,H_y+3,"%");
}
void getTime()
{
...
void dateDisplay(unsigned char x, unsigned char y)
{
  glcd_setfont(font5x7); 
  dayDisplay(x,y);
  glcd_putcharxy(x+18,y,48+(date/10)); 
...
{
getTime();
if(hour>17 || hour<6) glcd_putimage(57,2,moon,GLCD_PUTCOPY);
else glcd_putimage(57,2,sun,GLCD_PUTCOPY);
...
//glcd_line(8,18,119,18);
//glcd_line(5,47,122,47);  
//glcd_line(8,48,119,48);
if(alarm==255)
{
  alarm=0;
  min_set=0;
